package co.mobiwise.materialintro.utils;

/**
 * Created by mertsimsek on 24/01/16.
 */
public class Constants {

    public static final int DEFAULT_MASK_COLOR = 0x70000000;

    public static final long DEFAULT_DELAY_MILLIS = 0;

    public static final long DEFAULT_FADE_DURATION = 700;

    public static final int DEFAULT_TARGET_PADDING = 10;

    public static final int DEFAULT_COLOR_TEXTVIEW_INFO = 0xFF000000;

    public static final int DEFAULT_DOT_SIZE = 55;

}
